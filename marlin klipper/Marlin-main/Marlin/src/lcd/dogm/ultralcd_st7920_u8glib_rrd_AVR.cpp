...
void lcd_menu_dev_mode_status() {
  lcd.clear();
  lcd.print(F("Dev Mode: "));
  lcd.print(devModeEnabled ? F("Enabled") : F("Disabled"));
  lcd.setCursor(0, 1);
  lcd.print(F("Press [Right] to toggle"));
}

void lcd_menu_dev_mode_toggle() {
  devModeEnabled = !devModeEnabled;
  // Additional logic or actions can be added here
}

void lcd_menu_custom_option() {
  lcd.clear();
  lcd.print(F("Custom Option"));
  lcd.setCursor(0, 1);
  lcd.print(F("Dev Mode: "));
  lcd.print(devModeEnabled ? F("Enabled") : F("Disabled"));
  // Add more custom option logic here
}
...
uint8_t u8g_dev_rrd_st7920_128x64_fn(u8g_t *u8g, u8g_dev_t *dev, uint8_t msg, void *arg) {
  switch (msg) {
    case U8G_DEV_MSG_INIT: {
      ...
      lcd.add_menu(F("Dev Mode"), lcd_menu_dev_mode_status, LCD_ICON_SETTINGS);
      if (devModeEnabled) {
        lcd.add_menu(F("Custom Option"), lcd_menu_custom_option, LCD_ICON_SETTINGS);
    ...
    case U8G_DEV_MSG_STOP:
      // ... (your existing code for stopping)
      break;

    case U8G_DEV_MSG_PAGE_NEXT:
      // ... (your existing code for page navigation)
      break;

    // Add cases for other messages here as needed
  ...
  return u8g_dev_default_fn(u8g, dev, msg, arg);
}
